...
typedef enum mjtConstraintState_ { // constraint state
  mjCNSTRSTATE_SATISFIED =
      0,                  // constraint satisfied, zero cost (limit, contact)
...
struct mjContact_ { // result of collision detection functions
  ...
  mjtNum solreffriction[mjNREF]; // constraint solver reference, friction
                                 // directions
...
struct mjData_ {
  ...
  mjtSize maxuse_threadstack[mjMAXTHREAD]; // maximum stack allocation per
                                           // thread in bytes
  ...
  // solver statistics
  mjSolverStat solver[mjNISLAND *
                      mjNSOLVER]; // solver statistics per island, per iteration
  int solver_niter[mjNISLAND];    // number of solver iterations, per island
  int solver_nnz[mjNISLAND]; // number of nonzeros in Hessian or efc_AR, per
                             // island
  ...
  // mocap data
  mjtNum *mocap_pos; // positions of mocap bodies                        (nmocap
                     // x 3)
  ...
  // sleep state
  int *tree_asleep; // <0: awake; >=0: index cycle of sleeping trees    (ntree x
                    // 1)
  ...
  mjtNum *flexelem_aabb;  // flex element bounding boxes (center, size)
                          // (nflexelem x 6)
  ...
  int *ten_wrapadr; // start address of tendon's path                   (ntendon
                    // x 1)
  int *ten_wrapnum; // number of wrap points in path                    (ntendon
                    // x 1)
  ...
  mjtNum *
      wrap_xpos; // Cartesian 3D points in all paths                 (nwrap x 6)
  mjtNum *wrap_length; // arc length of wrap on surface (nwrap x 1)
  mjtNum *wrap_entry; // entry tangent point                              (nwrap
                      // x 3)
  mjtNum *
      wrap_exit; // exit tangent point                               (nwrap x 3)

  // computed by mj_fwdPosition/mj_transmission
  mjtNum *actuator_length; // actuator lengths (nu x 1)
  int *moment_rownnz; // number of non-zeros in actuator_moment row       (nu x
                      // 1)
  int *moment_rowadr; // row start address in colind array                (nu x
                      // 1)
  int *moment_colind; // column indices in sparse Jacobian                (nJmom
                      // x 1)
  ...
  mjtByte *
      bvh_active; // was bounding volume checked for collision        (nbvh x 1)

  // computed by mj_updateSleep
  int *tree_awake; // is tree awake; 0: asleep; 1: awake               (ntree x
                   // 1)
  int *body_awake; // body sleep state (mjtSleepState)                 (nbody x
                   // 1)
  int *body_awake_ind;   // indices of awake and static bodies (nbody x 1)
  int *parent_awake_ind; // indices of bodies with awake or static parents
                         // (nbody x 1)
  int *dof_awake_ind; // indices of awake dofs                            (nv x
                      // 1)
  ...
  mjtNum *qfrc_gravcomp; // passive gravity compensation force               (nv
                         // x 1)
  mjtNum
  ...
  mjtNum *qfrc_actuator; // actuator force                                   (nv
                         // x 1)
  ...
  mjtNum *
      qfrc_inverse; // net external force; should equal:
                    // qfrc_applied + J'*xfrc_applied + qfrc_actuator   (nv x 1)

  // computed by mj_sensorAcc/mj_rnePostConstraint if needed;
  // rotation:translation format
  ...
  mjContact
      *contact; // array of all detected contacts                   (ncon x 1)
  ...
  int *efc_J_rownnz; // number of non-zeros in constraint Jacobian row   (nefc x
                     // 1)
  int *efc_J_rowadr; // row start address in colind array                (nefc x
                     // 1)
  int *efc_J_rowsuper; // number of subsequent rows in supernode           (nefc
                       // x 1)
  int *
      efc_J_colind; // column indices in constraint Jacobian            (nJ x 1)
  ...
  int *tree_island; // island id of this tree; -1: none                 (ntree x
                    // 1)
  ...
  int *island_nv; // number of dofs in this island                    (nisland x
                  // 1)
  ...
  int *
      map_dof2idof; // map from dof to idof                             (nv x 1)
  int *
      map_idof2dof; // map from idof to dof;  >= nidof: unconstrained   (nv x 1)
  ...
  int *
      iM_rownnz; // inertia: non-zeros in each row                   (nidof x 1)
  int *
      iM_rowadr; // inertia: address of each row in iM_colind        (nidof x 1)
  ...
  int *island_ne; // number of equality constraints in island         (nisland x
                  // 1)
  int *island_nf; // number of friction constraints in island         (nisland x
                  // 1)
  int *island_nefc; // number of constraints in island                  (nisland
                    // x 1)
  int *island_iefcadr; // start address in iefc vector (nisland x 1)
  int *map_efc2iefc; // map from efc to iefc                             (nefc x
                     // 1)
  int *map_iefc2efc; // map from iefc to efc                             (nefc x
                     // 1)
  ...
  int *iefc_J_rownnz; // number of non-zeros in constraint Jacobian row   (nefc
                      // x 1)
  int *iefc_J_rowadr; // row start address in colind array                (nefc
                      // x 1)
  int *iefc_J_rowsuper; // number of subsequent rows in supernode (nefc x 1)
  int *iefc_J_colind; // column indices in constraint Jacobian            (nJ x
                      // 1)
  ...
  int *efc_AR_rownnz; // number of non-zeros in AR                        (nefc
                      // x 1)
  int *efc_AR_rowadr; // row start address in colind array                (nefc
                      // x 1)
  int *efc_AR_colind; // column indices in sparse AR                      (nA x
                      // 1)
  ...
  mjtNum
      *iefc_aref; // reference pseudo-acceleration                    (nefc x 1)
  int *
      iefc_state; // constraint state (mjtConstraintState)            (nefc x 1)
  mjtNum *
      iefc_force; // constraint force in constraint space             (nefc x 1)
...
// collision detection
typedef int (*mjfCollision)(const mjModel *m, const mjData *d, mjContact *con,
