...
// Tests for engine/engine_collision_continuous.c: the distance kernels, the per-pair gap and its
// gradient (finite-difference checked), the swept candidate generation, and the conservative
// advancement. Everything is exercised directly on hand-built pairs or tiny flex models; no
// contact solver is involved.
...
// vertex-triangle pair: the gap is the point-triangle distance (midsurface: radii not subtracted),
// and (n, cw) is its exact gradient, checked by central differences at every involved vertex
TEST_F(ContinuousCollisionTest, PairGapVertexTriangleGradient) {
  ...
  mjtNum rad[4] = {0.005, 0.005, 0.005, 0.005};
  mjcPair pair;
  pair.type = 0;
  ...
  mjtNum n[3], cw[4];
  int idv[4], nidx = 0;
  mjtNum g = mjc_pairGap(&pair, m, d, x, nullptr, nullptr, rad, n, idv, cw, &nidx, 1e30);
  EXPECT_NEAR(g, 0.5, 1e-12);                     // midsurface distance, radii not subtracted
  EXPECT_NEAR(n[0], 0, 1e-12);
  EXPECT_NEAR(n[1], 0, 1e-12);
  EXPECT_NEAR(std::fabs(n[2]), 1, 1e-12);
  EXPECT_GT(nidx, 0);
  ...
  for (int p = 0; p < nidx; p++) {
    for (int k = 0; k < 3; k++) {
      ...
      mjtNum gp = mjc_pairGap(&pair, m, d, x, nullptr, nullptr, rad, n, idv, cw, &nidx, 1e30);
      x[3*idv[p] + k] = saved - eps;
      mjtNum gm = mjc_pairGap(&pair, m, d, x, nullptr, nullptr, rad, n, idv, cw, &nidx, 1e30);
      x[3*idv[p] + k] = saved;
      mjtNum g0 = mjc_pairGap(&pair, m, d, x, nullptr, nullptr, rad, n, idv, cw, &nidx, 1e30);
      EXPECT_NEAR(cw[p]*n[k], (gp - gm) / (2*eps), 1e-6)
          << "gradient mismatch at involved vertex " << p << " axis " << k << " (gap " << g0 << ")";
    }
  }
  mj_deleteData(d);
  mj_deleteModel(m);
...
// a vertex sweeping through a triangle: the advance caps alpha so the gap keeps 20% of its value,
// reports the pair's own time of impact, and flags it as approaching; motion away is uncapped
TEST_F(ContinuousCollisionTest, AdvanceCapsCrossing) {
  ...
  int pt2flex[4] = {0, 1, 1, 1};           // cross-flex pair: no coherent-motion mean removal
  mjcPair cand;
  cand.type = 0;
  ...
  int idv[4], nidx = 0;
  mjtNum cgap[1];
  cgap[0] = mjc_pairGap(&cand, m, d, x, nullptr, nullptr, rad, n, idv, cw, &nidx, 1e30);
  ASSERT_NEAR(cgap[0], 0.5, 1e-12);

  // vertex 0 moves straight down by 1: the full step would end 0.5 below the triangle
  mjtNum dxw[12] = {0, 0, -1};
  int appr[1];
  mjtNum toi[1];
  mjtNum alpha = mjc_advance(m, d, x, dxw, nullptr, nullptr, rad, 4, fidx, &cand, 1,
                             cgap, pt2flex, appr, toi);
  // the advance stops when the gap has dropped to 20% of its value: alpha = (0.5 - 0.1)/1 = 0.4
  EXPECT_NEAR(alpha, 0.4, 1e-3);
  EXPECT_LT(toi[0], 1.0);
  EXPECT_EQ(appr[0], 1);

  // moving away at speed 1: the closing-rate bound is conservative (it does not project onto the
  // normal), so the pair still reaches the bisection and is flagged approaching -- but the actual
  // gap grows along the path, so the advance is uncapped and there is no impact
  mjtNum dxw_up[12] = {0, 0, +1};
  alpha = mjc_advance(m, d, x, dxw_up, nullptr, nullptr, rad, 4, fidx, &cand, 1,
                      cgap, pt2flex, appr, toi);
  EXPECT_NEAR(alpha, 1.0, 1e-12);
  EXPECT_NEAR(toi[0], 1.0, 1e-12);

  // slow motion (well under 80% of the gap): absorbed by the 20% floor without any bisection,
  // whatever its direction
  mjtNum dxw_slow[12] = {0, 0, -0.1};
  alpha = mjc_advance(m, d, x, dxw_slow, nullptr, nullptr, rad, 4, fidx, &cand, 1,
                      cgap, pt2flex, appr, toi);
  EXPECT_NEAR(alpha, 1.0, 1e-12);
  EXPECT_NEAR(toi[0], 1.0, 1e-12);
  EXPECT_EQ(appr[0], 0);
...
// two stacked cloths: the swept broad phase finds cross-flex pairs when they are within the
// detection reach and none when they are far apart
TEST_F(ContinuousCollisionTest, CandidatesFindApproachingPairs) {
  constexpr char xml[] = R"(
  <mujoco>
    <worldbody>
      <flexcomp name="lower" type="grid" dim="2" count="2 2 1" spacing="0.05 0.05 1"
                radius="0.005" mass="0.05" pos="0 0 0.5"/>
      <flexcomp name="upper" type="grid" dim="2" count="2 2 1" spacing="0.05 0.05 1"
                radius="0.005" mass="0.05" pos="0 0 %g"/>
    </worldbody>
  </mujoco>)";
  ...
  for (mjtNum dz : {0.002, 0.5}) {
    ...
    mjtNum band = 0.003;
    mjcPair cand[256];
    int ncand = mjc_candidates(m, d, x, nullptr, nullptr, 0, 0, rad, 3*band, 3*band, 0.0,
                               x, x, band, nfv, nfv, fidx, flist, fxadr, nfd, pt2flex,
                               cand, 256);
    if (dz < 0.01) {
      EXPECT_GT(ncand, 0) << "2 mm apart, within reach: pairs expected";
      for (int c = 0; c < ncand; c++) {
        EXPECT_TRUE(cand[c].type == 0 || cand[c].type == 1) << "flex-flex pair types only";
      }
    } else {
