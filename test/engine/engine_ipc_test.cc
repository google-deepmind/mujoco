...
// Total linear momentum, summed over the flex vertex bodies (each a point mass on three slide
// joints). Read from qvel rather than d->cvel, which is stale after mj_step.
...
// CONSERVATION under self-contact. Two cloths collide in mid-air with nothing else in the scene:
// no pins, no floor, no geoms, so contact, elastic and damping forces are all INTERNAL and cancel
// in the sum. Summing the step update over every vertex therefore gives, exactly,
//
//     sum_i m_i * (v_i+ - v_i)  ==  h * M_total * g
//
// independent of the contact solve -- PROVIDED the committed step is the solution of that solve.
// This is what makes the test worth its cost: a step that commits a line-search-truncated iterate
// discards the motion the truncation dropped, and the deficit lands here as missing momentum. That
// failure looks healthy to every other test we have (no NaN, no tunneling, plausible timings), so
// without this invariant a solver that silently stops advancing time passes CI.
TEST_F(IpcTest, SelfContactConservesMomentum) {
  ...
  // drive the upper cloth down onto the lower one: under uniform gravity alone the two would fall
  // together and never touch, so the approach velocity is what creates the contact
  ...
  ASSERT_GT(mass, 0) << "no flex vertex bodies found: the scene is not the one this test assumes";
  mjtNum h = m->opt.timestep;
  mjtNum scale = mass * mju_norm3(m->opt.gravity) * h;  // magnitude of one step's momentum change

  // 40 steps: the cloths meet at ~step 10 and stay in contact for the rest, so the window covers
  // approach, impact and sustained contact
  for (int s = 0; s < 40; s++) {
    mj_step(m, d);
    FlexMomentum(m, d, P1);
    ...
    for (int k = 0; k < 3; k++) {
      ...
      // tolerance is a fraction of ONE step's momentum change, so it stays meaningful under
      // sustained contact; the failure this guards discards most of a step and is O(1) relative
      EXPECT_NEAR(got, want, 1e-6 * scale)
          << "step " << s << " axis " << k << ": internal contact forces must cancel";
      P0[k] = P1[k];
    }
  }
  mj_deleteData(d);
...
// a cloth that STARTS inside a geom is pushed back out. Broad-phase must keep pairs whose gap is
// already negative: dropping them leaves the penetration with nothing acting on it and the cloth
// free-falls through.
TEST_F(IpcTest, ContactRecoversFromPenetration) {
  ...
  // every vertex starts 20 mm below the plane, so every flex-geom gap is negative at step 0
  for (int i = 0; i < 100; i++) {
    mj_step(m, d);
  }
  mj_kinematics(m, d);
