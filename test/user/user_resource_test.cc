...
TEST_F(ResourceTest, BuiltInMujocoAssetProviderRejectsUnsafeNames) {
  ScopedEnvVar assets_dir_env("MUJOCO_ASSETS_DIR", ".");

  mjResource* parent_resource =
      mju_openResource("", "mujoco:../CMakeLists.txt", nullptr, nullptr, 0);
  EXPECT_THAT(parent_resource, IsNull());

  mjResource* absolute_resource =
      mju_openResource("", "mujoco:/CMakeLists.txt", nullptr, nullptr, 0);
  EXPECT_THAT(absolute_resource, IsNull());
}
