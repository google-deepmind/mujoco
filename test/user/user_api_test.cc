...
TEST_F(MujocoTest, SignatureChangesWhenAddingElements) {
  ...
  // read through mjs_getSignature because the field is computed on demand
  uint64_t previous = mjs_getSignature(spec);
  auto expect_changed = [&](const char* what) {
    EXPECT_NE(mjs_getSignature(spec), previous) << what;
    EXPECT_NE(mjs_getSignature(spec), model->signature) << what;
    previous = mjs_getSignature(spec);
  };
