...
.. note::
   Speed up is bound to ``=``, not ``+``, and both speed shortcuts are ignored while ``Shift`` is held. On a US
   keyboard ``+`` is ``Shift`` ``=``, so pressing ``+`` does *not* speed up the simulation, despite what the ``F1``
   overlay suggests.
...
Flags with no shortcut assigned (Depth, Id Color and Cull Face) are omitted. Segment is also omitted: it declares
``,``, but that key is already claimed by the Activation visualization flag, which is registered first and therefore
wins.
