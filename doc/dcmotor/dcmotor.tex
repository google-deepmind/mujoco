...
\node[mode] at (0, 1.45) {$\begin{aligned}
  ...
  v &= \text{clip}(\tfrac{R}{K}\tau + K\dot\theta + v_{\text{raw}},\, \pm v_{\max}) \\[2pt]
...
terminal voltage, added to the controller output before the common saturation.
...
The computed controller voltage and the raw \texttt{voltage} input can exceed the physical driver's supply voltage. When \atVMAX{} is set ($v_{\max} > 0$), their sum is clamped before the motor equations:
\begin{equation*}
  v \leftarrow \text{clip}(v, \pm v_{\max})
  \label{eq:vlimit}
\end{equation*}
This differs from \texttt{ctrlrange} (clamping individual user commands $u$) and \texttt{forcerange} (clamping output torque). In raw voltage mode, both \texttt{ctrlrange} and \atVMAX{} bound the voltage; if both are set, the tighter limit wins. Because the torque-to-voltage map \eqref{eq:torque_to_voltage} compensates back-EMF, the achievable torque under \atVMAX{} shrinks with speed: the torque-speed envelope of \S\ref{sec:electromagnetics}.
