...
// In order to support older APIs (namely mj_addFileVFS and mj_addBufferVFS),
// we create custom providers dynamically that are mounted to the VFS.
template <typename Provider, typename... Args>
static int MountCustomProvider(mjVFS* vfs, Args&&... args) {
  mujoco::user::VFS* impl = mujoco::user::VFS::Upcast(vfs);
  if (impl == nullptr) {
    mju_error("mjVFS is null.");
    return -1;
...
// Custom provider for mj_addFileVFS.
class FileProvider : public mjpResourceProvider {
 public:
  FileProvider(const char* dir, const char* filename) {
    mjp_defaultResourceProvider(this);

    file_path = mujoco::user::FilePath(dir ? dir : "", filename);
    mount_path = file_path.StripPath().Lower();

    open = [](mjResource* res) {
      FileProvider* self = (FileProvider*)res->provider;
      return OpenFile(self->file_path.c_str(), res);
    };
    read = [](mjResource* res, const void** buffer) {
      FileProvider* self = (FileProvider*)res->provider;
      return ReadFile(self->file_path.c_str(), res, buffer);
    };
    close = [](mjResource* res) { CloseFile(res); };
    modified = [](const mjResource* res, const char* time) {
      return FileModified(res, time);
    };
  }

  const mujoco::user::FilePath& MountPath() const { return mount_path; }

  mujoco::user::FilePath file_path;
  mujoco::user::FilePath mount_path;
};

// Custom provider for mj_addBufferVFS.
class BufferProvider : public mjpResourceProvider {
 public:
  BufferProvider(const char* name, const void* src, size_t n) {
    mjp_defaultResourceProvider(this);

    path = mujoco::user::FilePath(name);

    static constexpr std::uint64_t prime = 0x100000001b3;
    hash = n ? 0xcbf29ce484222325 : 0;

    const std::uint8_t* src_bytes = static_cast<const std::uint8_t*>(src);
    contents.reserve(n);
    for (size_t i = 0; i < n; i++) {
      contents.push_back(src_bytes[i]);
      hash |= src_bytes[i];
      hash *= prime;
    }
    ...
    open = [](mjResource* res) {
      ...
      mju_encodeBase64(res->timestamp, (std::uint8_t*)&self->hash,
                       sizeof(self->hash));
      return 1;
    };
    read = [](mjResource* res, const void** out) {
      BufferProvider* self = (BufferProvider*)res->provider;
      *out = reinterpret_cast<void*>(self->contents.data());
      return static_cast<int>(self->contents.size());
    };
    close = [](mjResource* res) {
      // no-op
    };
    modified = [](const mjResource* res, const char* timestamp) {
  ...
  const mujoco::user::FilePath& MountPath() const { return path; }

  mujoco::user::FilePath path;
...
int mj_addFileVFS(mjVFS* vfs, const char* directory, const char* filename) {
  return MountCustomProvider<FileProvider>(vfs, directory, filename);
}

int mj_addBufferVFS(mjVFS* vfs, const char* name, const void* buffer,
                    int nbuffer) {
  return MountCustomProvider<BufferProvider>(vfs, name, buffer, nbuffer);
}
