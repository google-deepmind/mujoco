...
void mjCFlexcomp::LoadGMSH41(
  ...
  if (binary == 0) {
    ...
    if (!ss.good()) { throw mjCError(NULL, "Error reading Nodes header"); }

    // check number of nodes is a positive number
    if (numNodes < 0) { throw mjCError(NULL, "Invalid number of nodes"); }
    ...
    if (entityDim < 1 || entityDim > 3) { throw mjCError(NULL, "Entity must be 1D, 2D or 3D"); }
    ...
    for (size_t i = 0; i < numNodes; i++) {
      size_t tag;
      ss >> tag;
      if (!ss.good()) { throw mjCError(NULL, "Error reading node tags"); }
      if (tag != i + minNodeTag) { throw mjCError(NULL, "Node tags must be sequential"); }
    }
    ...
    for (size_t i = 0; i < 3 * numNodes; i++) {
      double x;
      ss >> x;
      if (!ss.good()) { throw mjCError(NULL, "Error reading node coordinates"); }
      point.push_back(x);
    }
  }
  // binary nodes
    ...
    // check header size
    if (nodeend - nodebegin < kGmsh41HeaderSize) { throw mjCError(NULL, "Invalid nodes header"); }
  ...
  if (binary == 0) {
    ...
    if (!ss.good()) { throw mjCError(NULL, "Error reading Elements header"); }
    ...
    if (numElements < 0) { throw mjCError(NULL, "Invalid number of elements"); }
    ...
    for (size_t i = 0; i < numElements; i++) {
      ...
      for (int k = 0; k <= entityDim; k++) {
        ss >> nodeid;
        if (!ss.good()) { throw mjCError(NULL, "Error reading Elements"); }
...
void mjCFlexcomp::LoadGMSH22(
  ...
  if (binary == 0) {
    ...
    std::getline(ss, line);
    if (!IsValidElementOrNodeHeader22(line)) { throw mjCError(NULL, "Invalid node header"); }
  ...
  // binary nodes
    ...
    // check header size
    if (nodeend - nodebegin < nodeHeaderSize) { throw mjCError(NULL, "Invalid nodes header"); }
    ...
    } catch (const std::out_of_range& e) { throw mjCError(NULL, "Invalid number of nodes"); }
    numNodes = maxNodeTag;
  ...
  if (binary == 0) {
    ...
    // checking header template
    std::getline(ss, line);
    if (!IsValidElementOrNodeHeader22(line)) { throw mjCError(NULL, "Invalid elements header"); }
    ss.seekg(-(line.size() + 1), std::ios::cur);
    // read header
    ...
    if (numNodeTags < 1 || numNodeTags > 4) { throw mjCError(NULL, "Invalid number of node tags"); }
    ...
    for (size_t i = 0; i < numElements; i++) {
      ...
      for (int k = 0; k < numNodeTags; k++) {
        ss >> nodeTag;
        if (!ss.good()) { throw mjCError(NULL, "Error reading Elements"); }
        if (nodeTag > numNodes || nodeTag < 1) { throw mjCError(NULL, "Invalid node tag"); }
  ...
  // binary elements
    ...
    } catch (const std::out_of_range& e) { throw mjCError(NULL, "Invalid number of elements"); }
    ...
    int elementsBufferSizeGmshApp = elementHeaderSizeGmshApp + numElements * elementDataSizeGmshApp;
    ...
    if (numTags > 0) {
      ...
      for (int i = 1; i < numElements; i++) {
        ...
        for (int k = 0; k < numNodeTags; k++) {
          ...
          if (nodeTag > numElements || nodeTag < 1) { throw mjCError(NULL, "Invalid node tag"); }
          element.push_back(nodeTag - 1);
        }
      }
    }
