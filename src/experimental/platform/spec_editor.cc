...
mjSpec* SpecEditor::GetActiveSpec() const {
  return active_spec_.get();
}
...
mjsElement* SpecEditor::GetActiveElement() const {
  return active_element_;
}

mjsElement* SpecEditor::GetRefElement() const {
  return ref_element_;
}
...
void SpecEditor::Undo() {
  if (CanUndo()) {
    ...
    active_element_ = active_map_.Resolve(active_spec_.get(), active_element_key_);
    UpdateReferenceElement();
  }
}

bool SpecEditor::CanUndo() const {
  return cursor_ > 0;
}
...
bool SpecEditor::CanRedo() const {
  return cursor_ < history_.size() - 1;
}
