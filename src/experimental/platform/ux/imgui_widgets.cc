...
void ImGui_DataPtrTable::DataPtr(const char* label, const double* ptr,
                                 int index, int n) {
  Numeric(label, ptr, index, n);
}

template <typename T>
void ImGui_DataPtrTable::Numeric(const char* label, const T* ptr, int index,
                                 int n) {
  ...
  constexpr const char* fmt1 = std::is_floating_point_v<U> ? "%f" : "%d";
  ...
  auto text1 = [&](int offset) { ImGui::Text(fmt1, (U)(addr[offset])); };
...
void ImGui_SpecElementTable::operator()(const char* name, const char* alt_name,
  ...
  switch (alt.type) {
    ...
    case mjORIENTATION_AXISANGLE:
      (*this)(aname("axisangle").c_str(), alt.axisangle, ref_alt.axisangle,
              tooltip);
      break;
