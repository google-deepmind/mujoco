...
// normal. IPC-specific: helper for ipc_geomDist's convex-mesh SDF branch; the engine has no
...
static void ipc_closestOnPoly(const mjtNum* p, const float* vbase, const int* pv, int nv,
  ...
  mjtNum pp[3];
  for (int k = 0; k < 3; k++)
    pp[k] = p[k] - dpl * nrm[k];  // projection onto face plane
  int npos = 0, nneg = 0;
  for (int i = 0; i < nv; i++) {
...
static mjtNum ipc_geomDist(const mjModel* m, int gi, const mjtNum* gpos, const mjtNum* gmat,
  ...
  if (type == mjGEOM_MESH) {
    ...
    for (int p = 0; p < pn; p++) {
      ...
      mjtNum cc[3];
      ipc_closestOnPoly(pl, vbase, pv, nv, pnl, cc);
    ...
    mjtNum nl[3];
    for (int k = 0; k < 3; k++)
      nl[k] = (dist > 1e-12) ? (pl[k] - bc[k]) / dist : bestn[k];
  ...
  switch (con->type) {
    ...
    case 1: {  // edge-edge contact (edge a1b1 against edge a2b2): flex self OR cross-flex
      ...
      mjtNum cp1[3], cp2[3], st[2],
          dd = ipc_segSeg(&x[3 * a1], &x[3 * b1], &x[3 * a2], &x[3 * b2], cp1, cp2, st);
      for (int k = 0; k < 3; k++)
        n[k] = (cp1[k] - cp2[k]) / dd;
      idv[0] = a1;
      idv[1] = b1;
...
// no radius (excluded by ipc_conVerts), so geom/flex and sphere/flex contacts keep the global ghat.
static mjtNum ipc_conGhat(const ipcCon* con, const mjtNum* rad, mjtNum ghat) {
  int vv[4], nvv;
  ipc_conVerts(con, vv, &nvv);
  mjtNum g = ghat;
  for (int q = 0; q < nvv; q++)
...
static mjtNum ipc_conGapAdv(const ipcCon* con, const mjModel* m, const mjData* d, const mjtNum* x,
  ...
  for (int q = 0; q < nv; q++) {
    int fq = fidx[v[q]];
    for (int k = 0; k < 3; k++)
      P[q][k] = x[3 * v[q] + k] + (fq >= 0 ? t * dxw[3 * fq + k] : 0.0);
  }
  mjtNum cp[3], w[3], c1[3], c2[3], st[2];
  switch (con->type) {
    case 0:
      return ipc_ptTri(P[0], P[1], P[2], P[3], cp, w) - (rad[con->idx[0]] + rad[con->idx[1]]);
    case 1:
      return ipc_segSeg(P[0], P[1], P[2], P[3], c1, c2, st) - (rad[con->idx[0]] + rad[con->idx[2]]);
    case 2: {
      mjtNum nn[3];
      return ipc_geomDist(m, con->gi, d->geom_xpos + 3 * con->gi, d->geom_xmat + 9 * con->gi, P[0],
                          nn, 1e30) -
             rad[con->idx[0]];
    }
    case 3:
      return ipc_ptTri(&gv[3 * con->idx[0]], P[0], P[1], P[2], cp, w) - rad[con->idx[1]];
    default: {
      const mjtNum* eg = &ge[6 * con->idx[0]];
      return ipc_segSeg(eg, eg + 3, P[0], P[1], c1, c2, st) - rad[con->idx[1]];
    }
  }
}
...
static void ipc_try(ipcCon con, const mjtNum* x, const mjtNum* xfree, const mjtNum* rad,
  ...
  mjtNum craw =
      con.ld0 - ipc_off(ipc_conGhat(&con, rad, ghat));  // c_raw(x) (un-baked linearized gap)
...
static void ipc_updateSlack(ipcCon* cand, int ncand, const int* held, const mjtNum* x,
  ...
  for (int c = 0; c < ncand; c++) {
    ...
    mjtNum craw = con->ld0 - ipc_off(ipc_conGhat(
...
static void ipc_mergeActiveSet(ipcCon* aset, int* naset, const ipcCon* cand, int ncand,
  ...
  for (int c = 0; c < ncand; c++) {
    ...
    ipcCon con = cand[c];
    int vv[4], nvv;
    ipc_conVerts(&con, vv, &nvv);  // warm-start lam from the per-point binding multiplier
    mjtNum s = 0;
    for (int q = 0; q < nvv; q++)
...
static void ipc_addCand(ipcCon con, const mjModel* m, const mjData* d, const mjtNum* x,
                        ...
                        mjtNum thresh, const mjtNum* dfrom, const mjtNum* dto, mjtNum ghat,
  ...
  for (int p = 0; p < nidx; p++) {
    ...
    if (vp < 0) continue;
    for (int c = 0; c < 3; c++)
      rel[c] += cw[p] * (dto[3 * vp + c] - dfrom[3 * vp + c]);
  }
...
static int ipc_candidates(const mjModel* m, const mjData* d, const mjtNum* x, const mjtNum* gv,
                          const mjtNum* ge, int ngv, int nge, mjtNum r, const mjtNum* rad,
                          mjtNum thresh, mjtNum threshGeom, mjtNum maxdisp, const mjtNum* dfrom,
  ...
  // Each query is against one flex's element BVH (ipc_bvhBox); triangle/edge vertices are mapped
  ...
  for (int k = 0; k < nfd; k++) {  // query flex fk's element BVH
    ...
    for (int c = 0; c < nge; c++) {
      ...
      int n = ipc_bvhBox(m, d, fk, qc, qh, stk, outel, ne_k);
      qid++;
      for (int i = 0; i < n; i++) {
        int e = outel[i];
        for (int j = 0; j < 3; j++) {
          ...
          ipcCon con = {4,
                        ...
                        -1};
          ipc_addCand(con, m, d, x, gv, ge, r, rad, threshGeom, dfrom, dto, ghat, cand, &nc,
                      candmax);
        }
      }
    }
    ...
    for (int v = 0; v < nfv; v++) {
      ...
      mjtNum thv = 3.0 * ipc_fmin(r, ipc_fmin(rad[v], rk)) +
                   4.0 * maxdisp;  // per-pair band: thinner flex sets it
...
void mj_IPC(const mjModel* m, mjData* d) {
  ...
  // appr is filled by ipc_ccd; actc/actpt are now unused but kept allocated to avoid churning the
  // frees.
  ...
  int ncand = ipc_candidates(m, d, x, gv, ge, ngv, nge, r, rad, thresh, threshGeom, maxdisp, xold,
  ...
  for (int c = 0; c < ncand;
    ...
    int vv[4], nvv;
    ipc_conVerts(&cand[c], vv, &nvv);  // (binding = max over its free-point participants)
    mjtNum s = 0;
    for (int q = 0; q < nvv; q++)
      if (g_pal[vv[q]] > s) s = g_pal[vv[q]];
    cand[c].lam = s;
  ...
  // FLEX: SEED the persistent active set from the iter-0 broad-phase. ipc_addCand already pruned
  ...
  for (int outer = 0; outer < outer_cap && N_total > 0;
    ...
    for (int c = 0; c < wn; c++)
      wcon[c].ld0 = ipc_conGap(&wcon[c], m, d, xfree, gv, ge, r, rad, wcon[c].ln, wcon[c].liv,
    ...
    ncand = ipc_candidates(m, d, xfree, gv, ge, ngv, nge, r, rad, 3 * ghat, 3 * ghat, 0.0, xfree, x,
    ...
    mjtNum ac =
        ipc_ccd(m, d, xfree, dx, gv, ge, r, rad, nfv, fidx, cand, ncand, cgap, pt2flex, appr);
...
// world-space sharp vertices / edges of geom gi at d's pose (out sized by caller); returns the
// count
