...
// Does the weld parent have independent, ordered XYZ slides? Used only to select fast paths;
...
int mj_flexBodySimple(const mjModel* m, int body) {
  ...
  for (int j=0; j < 3; j++) {
    ...
    for (int k=0; k < 3; k++) {
      if (mju_abs(m->jnt_axis[3*(jadr+j)+k] - (j == k)) > 1e-14) return 0;
    }
  }
  return 1;
}
...
// Can this standard flex use the fixed-frame, three-DOF assembly and cached factor?
