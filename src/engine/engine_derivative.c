...
// the contact stiffness published for this solve (see mjEffContact), NULL if none
static const mjEffContact* effContact(const mjData* d) {
  return (const mjEffContact*)d->efm_contact;
}


void mjd_flexContact_mul(const mjModel* m, mjData* d, mjtNum* res, const mjtNum* vec,
                         mjtNum scale) {
  const mjEffContact* es = effContact(d);
  if (!es) {
    return;
  }
  // callers ask for scale*K_contact; D carries the metric's h^2 (see mjEffContact), so divide it
  // back out to land on K rather than h^2*K
  mjtNum ih2 = 1 / (m->opt.timestep * m->opt.timestep);
  for (int c = 0; c < es->npair; c++) {
    const int* bp = es->base + mjNEFFCONPT*c;
    const mjtNum* wp = es->w + 3*mjNEFFCONPT*c;
    int n = es->npt[c];
    mjtNum wv = 0;
    for (int p = 0; p < n; p++) {
...
// Contact enters here too, as the published pair list rather than as CSR entries (see
// mjEffContact): it is a sum of rank-1 terms, so applying it costs O(npt) per pair.
...
int mjd_effContactCount(const mjData* d) {
  const mjEffContact* e = effContact(d);
...
// Mtilde = M + K carries inertia and flex elasticity but no contact. Where contact is the stiffest
// coupling in the system, preconditioning with Mtilde alone leaves the solve badly conditioned, so
// both contact families are folded in: the published pair list and the ordinary efc rows, each
// contributing D*J^T*J. Only the per-vertex 3x3 diagonal survives into the blocks, and the
// off-diagonal vertex-vertex coupling is dropped, as it is for the elastic part. The efc side is
// what CGBlockBuild does for metric-free models; that path excludes metric-active ones, which is
// why the fold is repeated here.
int mjd_effPrecContact(const mjModel* m, mjData* d, mjtNum* L,
  ...
  for (int c=0; es && c < es->npair; c++) {
    int n = es->npt[c];
    for (int q=0; q < n; q++) {
      int b = es->base[mjNEFFCONPT*c + q], k = blk[b];
      if (k < 0) {
        continue;
      }
      const mjtNum* w = es->w + 3*mjNEFFCONPT*c + 3*q;
      for (int a=0; a < 3; a++) {
...
void mjd_effContactMulAdd(const mjData* d, mjtNum* res, const mjtNum* vec) {
  const mjEffContact* e = effContact(d);
  if (!e) {
    return;
  }
  for (int c=0; c < e->npair; c++) {
    int n = e->npt[c];
    const int* base = e->base + mjNEFFCONPT*c;
    const mjtNum* w = e->w + 3*mjNEFFCONPT*c;
    mjtNum s = 0;
    for (int p=0; p < n; p++) {
...
void mjd_effMulAdd(const mjModel* m, mjData* d, mjtNum* res, const mjtNum* vec,
  ...
  if (!d->efm_active) {
    if (flg_contact) {
      mjd_effContactMulAdd(d, res, vec);
    }
...
//
// D carries the h^2 the assembler used to apply as s1, per the mjEffContact convention: D*w*w^T is
...
void mjd_flexContactPairs(const mjModel* m, mjData* d) {
  ...
  int nv = m->nv;
  mjtNum h = m->opt.timestep;
  mjEffContact* es = EFMALLOC(mjEffContact, 1);
  int* npt   = EFMALLOC(int, npair);
  int* base  = EFMALLOC(int, mjNEFFCONPT*npair);
  mjtNum* w  = EFMALLOC(mjtNum, 3*mjNEFFCONPT*npair);
  mjtNum* D  = EFMALLOC(mjtNum, npair);
  mjtNum* f  = EFMALLOC(mjtNum, nv);
  mju_zero(w, 3*mjNEFFCONPT*npair);
  mju_zero(f, nv);
  ...
  for (int i = 0; i < d->ncon; i++) {
    ...
    int gv[mjNEFFCONPT], ngv = contactFlexVerts(m, con, gv, mjNEFFCONPT);
    if (!ngv) {
      continue;
    }
    ...
    int* bp = base + mjNEFFCONPT*np;
    mjtNum* wp = w + 3*mjNEFFCONPT*np;
    int n = 0;
    for (int q = 0; q < ngv; q++) {
