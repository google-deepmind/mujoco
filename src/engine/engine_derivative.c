...
void mjd_flexStretch_mul(const mjModel* m, mjData* d, mjtNum* res, const mjtNum* vec,
  ...
  for (int f = 0; f < m->nflex; f++) {
    ...
    for (int t = 0; t < elemnum; t++) {
      ...
      // unpack triangular metric (same layout as mj_flexPassiveStretch: 21 numbers per element)
      mjtNum metric[36];
      int id = 0;
...
// is this interp flex processed by mjd_flexInterp_mul? (mirrors its skip logic)
static int flexInterp_processed(const mjModel* m, int f) {
  if (!m->flex_interp[f]) {
    return 0;
  }
...
int mjd_flexInterpAssemblable(const mjModel* m) {
  for (int f = 0; f < m->nflex; f++) {
...
#define FLEXINTERP_WALK(f, body) { \
    ...
    int npe = shell_ ? (order_+1)*(order_+1) : (order_+1)*(order_+1)*(order_+1); \
    int nelem_fe_ = shell_ ? 2*(cy_*cz_ + cx_*cz_ + cx_*cy_) : cx_*cy_*cz_; \
    ...
    for (int fe = 0; fe < nelem_fe_; fe++) { \
      ...
      if (shell_) { \
        ...
        mju_flexGatherCellState(order_, cy_, cz_, ci_, cj_, ck_, NULL, NULL, NULL, \
                                NULL, NULL, NULL, gindices, NULL); \
      } \
      body \
    } \
...
int mjd_flexStiff_any(const mjModel* m, int flg_interp) {
  for (int f = 0; f < m->nflex; f++) {
...
static int flexStiff_active(const mjModel* m, int f, int flg_bend, int flg_stretch) {
  if (m->flex_interp[f] || m->flex_rigid[f] || m->flex_dim[f] < 2) {
    return 0;
  }
...
int mjd_flexStiff_assemble(const mjModel* m, mjData* d, int* rownnz, int* rowadr,
  ...
  #define FLEXSTIFF_BLOCK(si, sj, pos) { \
    ...
    while (lo <= hi) { \
      int mid = (lo + hi)/2; \
      if (vdof[neigh[mid]] == vdof[sj]) { pos = mid - nadr[si]; break; } \
      if (vdof[neigh[mid]] < vdof[sj]) lo = mid + 1; else hi = mid - 1; \
    } \
  }
...
void mjd_effSolve(const mjModel* m, mjData* d, mjtNum* x, const mjtNum* b) {
  ...
  // relative tolerance on the residual (matches the post-hoc 1e-10; a looser solve was measured
  // a wash: soft models gain ~15% but stiff scenes pay 4x in outer iterations)
