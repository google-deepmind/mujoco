...
static void PrimalAllocate(const mjModel* m, mjData* d, mjPrimalContext* ctx, int flg_Newton) {
  ...
  if (d->efm_active && ctx->island < 0 && !is_elliptic && mjd_effContactCount(d)) {
    nNum += 9*d->nefmdof;
  }
...
static void mj_solPrimal(const mjModel* m, mjData* d, int island, int maxiter, int flg_Newton) {
  // the published contact stiffness rides with the metric operator, which only the CG path
  // applies; the Newton Hessian does not include it, so force CG whenever contact is published.
  if (mjd_effContactCount(d) && island < 0) {
    flg_Newton = 0;
  }
  int iter = 0;
  mjtNum alpha, beta;
