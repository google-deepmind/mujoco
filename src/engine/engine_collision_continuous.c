...
// Continuous-collision geometry for deformables; see the header for scope.
...
// early-exits on margin so it can't return the unconditional signed distance this module uses in
...
// (engine_collision_primitive.c:442-470, identical x1/x2 clip). Kept separate because contact-gen returns
// (s,t) (mjraw_ discards them into mjraw_SphereSphere), the PARALLEL branch diverges (mjraw_ emits
// up to TWO contacts at the overlap ends -- a contact-gen stability trick -- vs the one pair +
...
// normal. Helper for mjc_GeomDist's convex-mesh SDF branch; the engine has no
...
// A continuous-contact solver needs a differentiable POINT-to-geom signed distance;
...
mjtNum mjc_standoff(mjtNum band, mjtNum cap) { return band < cap ? band : cap; }
...
// emitting NaN; it is re-linearized at the next configuration. The CCD certificate keeps dd > 0,
// so this only guards configurations that are already degenerate on entry.
static void gapNormal(mjtNum* n, const mjtNum* p1, const mjtNum* p2, mjtNum dd) {
  if (dd < mjMINVAL) {
    n[0] = n[1] = n[2] = 0;
    return;
  }
...
// The engine's collision generators only emit an mjContact (dist/normal); this
// returns gap + normal
// + barycentric weights cw (dg/dx) for the barrier gradient/Hessian, which they do not expose.
mjtNum mjc_pairGap(const mjcPair* con, const mjModel* m, const mjData* d, const mjtNum* x,
...
void mjc_pairVerts(const mjcPair* con, int* v, int* nv) {
  switch (con->type) {
    case 0:
    case 1:
      v[0] = con->idx[0];
...
// no radius (excluded by mjc_pairVerts), so geom/flex and sphere/flex contacts keep the global ghat.
mjtNum mjc_pairBand(const mjcPair* con, const mjtNum* rad, mjtNum ghat) {
  int vv[4], nvv;
  mjc_pairVerts(con, vv, &nvv);
  mjtNum g = ghat;
  for (int q=0; q < nvv; q++)
...
static mjtNum conGapAdv(const mjcPair* con, const mjModel* m, const mjData* d, const mjtNum* x,
                        const mjtNum* dxw, mjtNum t, const mjtNum* gv, const mjtNum* ge,
                        const mjtNum* rad, const int* fidx) {
  int v[4], nv;
  mjtNum P[4][3];
  mjc_pairVerts(con, v, &nv);
  for (int q=0; q < nv; q++) {
    int fq = fidx[v[q]];
  ...
  switch (con->type) {
    case 0:
      return mjc_PtTri(P[0], P[1], P[2], P[3], cp, w);  // midsurface, matching mjc_pairGap
    case 1:
      return mjc_SegSeg(P[0], P[1], P[2], P[3], c1, c2, st);  // midsurface, matching mjc_pairGap
    case 2: {
      mjtNum nn[3];
...
mjtNum mjc_advance(const mjModel* m, const mjData* d, const mjtNum* x, const mjtNum* dxw,
  ...
  if (toiout)
    for (int c=0; c < ncand; c++)
      toiout[c] = 1.0;  // Alg.3: per-pair collision time, 1 == does not collide this step
  for (int c=0; c < ncand; c++) {
    const mjcPair* con = &cand[c];
    ...
    if (g0 <= 0) continue;  // already at/under the surface: the contact solver owns it
    if (l <= 0.8 * g0)
    ...
    for (int it=0; it < 32; it++) {
      mjtNum g = conGapAdv(con, m, d, x, dxw, t, gv, ge, rad, fidx);
...
static mjtNum bandOffset(const mjcPair* con, const mjtNum* rad) {
  switch (con->type) {
    case 0:
      return rad[con->idx[0]] + rad[con->idx[1]];  // vertex + triangle flex radius
    case 1:
      return rad[con->idx[0]] + rad[con->idx[2]];  // both edges (flex) radii
    default:
...
static void addCand(mjcPair con, const mjModel* m, const mjData* d, const mjtNum* x,
                    const mjtNum* gv, const mjtNum* ge, const mjtNum* rad, mjtNum thresh,
                    const mjtNum* dfrom, const mjtNum* dto, mjtNum ghat, mjcPair* cand, int* nc,
                    int candmax) {
  if (*nc >= candmax) return;
  mjtNum n[3], cw[4];
  int idv[4], nidx;
  mjtNum g = mjc_pairGap(&con, m, d, x, gv, ge, rad, n, idv, cw, &nidx, thresh);
  // DETECTION band. A flex-flex gap is now the midsurface distance, so it reads r1+r2 larger than
  // the pre-midsurface convention; add that back HERE (and only here) so the broad-phase reaches
  // exactly as far as it used to. This offset belongs to the band, NOT to the rest target
  // -- the rest target (mjc_standoff(mjc_pairBand)) is unchanged and must stay that way.
  mjtNum bo = bandOffset(&con, rad);
  if (g >= bo + thresh) return;
  // KEEP penetrating geom pairs (g < 0): the normal is still defined there and the solver's restoring
  // force is what pushes the flex back out, so dropping them leaves the penetration unopposed;
  ...
  if (g < bo + mjc_pairBand(&con, rad, ghat) + sqrt(mju_dot3(rel, rel))) cand[(*nc)++] = con;
}
...
int mjc_candidates(const mjModel* m, const mjData* d, const mjtNum* x, const mjtNum* gv,
  ...
  for (int gi=0; gi < m->ngeom; gi++) {  // free point (flex vert) vs STATIC geom
    ...
    for (int v=0; v < nfv; v++) {  // flex verts only (rigid bodies carry no hard barrier)
      ...
      mjcPair con = {2, {v, 0, 0, 0}, gi};
      addCand(con, m, d, x, gv, ge, rad, thresh, dfrom, dto, ghat, cand, &nc, candmax);
    }
  }

  // (Rigid sphere-sphere and sphere-vs-flex HARD contacts removed in increment A: rigid bodies live
  // in their own solver block and carry no continuous contact yet. Soft flex-rigid contact is a
  ...
  for (int k=0; k < nfd; k++) {  // query flex fk's element BVH
    ...
    for (int c=0; c < ngv; c++) {
      ...
      for (int i=0; i < n; i++) {
        ...
        addCand(con, m, d, x, gv, ge, rad, threshGeom, dfrom, dto, ghat, cand, &nc, candmax);
      }
    }
    ...
    for (int v=0; v < nfv; v++) {
      ...
      mjtNum thv = 3.0 * min2(ghat, min2(rad[v], rk)) + 4.0 * maxdisp;
      ...
      for (int i=0; i < n; i++) {
        ...
        mjcPair con = {0, {v, A, B, C}, -1};
        addCand(con, m, d, x, gv, ge, rad, thv, dfrom, dto, ghat, cand, &nc, candmax);
      }
    }
    ...
    for (int kj=0; kj <= k; kj++) {
      ...
      for (int e1=0; e1 < en_j; e1++) {
        ...
        mjtNum the = 3.0 * min2(ghat, min2(rad[a1], rk)) + 4.0 * maxdisp;  // per-pair band
        mjtNum qc[3], qh[3];
        for (int kk=0; kk < 3; kk++) {
        ...
        for (int i=0; i < n; i++) {
          ...
          for (int j=0; j < 3; j++) {
            ...
            mjcPair con = {1, {a1, b1, a2, b2}, -1};
            addCand(con, m, d, x, gv, ge, rad, the, dfrom, dto, ghat, cand, &nc, candmax);
          }
        }
      }
    }
