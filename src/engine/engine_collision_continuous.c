...
static mjtNum conGapAdv(const ipcCon* con, const mjModel* m, const mjData* d, const mjtNum* x,
  ...
  switch (con->type) {
    case 0:
      return mjc_PtTri(P[0], P[1], P[2], P[3], cp, w);  // midsurface, matching mjc_conGap
    case 1:
      return mjc_SegSeg(P[0], P[1], P[2], P[3], c1, c2, st);  // midsurface, matching mjc_conGap
    case 2: {
      mjtNum nn[3];
...
// inside mjc_conGap, so they contribute nothing here.
static mjtNum bandOffset(const ipcCon* con, const mjtNum* rad) {
  switch (con->type) {
    case 0:
    ...
    default:
      return 0.0;  // geom side: mjc_conGap already subtracted the flex radius for types 2/3/4
  }
}
...
static void addCand(ipcCon con, const mjModel* m, const mjData* d, const mjtNum* x,
  ...
  mjtNum n[3], cw[4];
  int idv[4], nidx;
  mjtNum g = mjc_conGap(&con, m, d, x, gv, ge, rad, n, idv, cw, &nidx, thresh);
  ...
  // -- the rest target (mjc_off(mjc_conGhat)) is unchanged and must stay that way.
  mjtNum bo = bandOffset(&con, rad);
  if (g >= bo + thresh) return;
  // KEEP penetrating geom pairs (g < 0): the normal is still defined there and the AL restoring
  ...
  if (g < bo + mjc_conGhat(&con, rad, ghat) + sqrt(mju_dot3(rel, rel))) cand[(*nc)++] = con;
}
...
int mjc_candidates(const mjModel* m, const mjData* d, const mjtNum* x, const mjtNum* gv,
  ...
  for (int k=0; k < nfd; k++) {  // query flex fk's element BVH
    ...
    for (int v=0; v < nfv; v++) {
      ...
      mjtNum thv = 3.0 * min2(IPC_GHAT, min2(rad[v], rk)) + 4.0 * maxdisp;
    ...
    for (int kj=0; kj <= k; kj++) {
      ...
      for (int e1=0; e1 < en_j; e1++) {
        ...
        mjtNum the = 3.0 * min2(IPC_GHAT, min2(rad[a1], rk)) + 4.0 * maxdisp;  // per-pair band
        mjtNum qc[3], qh[3];
        for (int kk=0; kk < 3; kk++) {
