...
// Contact DETECTION band. Unlike the standoff this is not a physical length: the AL constraint has
// no activation distance (the active set is admitted by CCD time-of-impact, not by proximity), so
// this only bounds how early the broad phase starts tracking a pair and how long the linearized
// gap keeps it held. It is a constant rather than a radius because after the midsurface change the
// flex radius no longer offsets a flex-flex distance, and any radius that entered here also
// entered the standoff through mjc_conGhat's min -- so a thin flex silently shrank every other
// pair's rest gap. Keeping it >= IPC_DELTACAP makes that min collapse to min(radii, IPC_DELTACAP),
// which is the standoff as documented above. Wider costs candidates, narrower holds pairs later.
#define IPC_GHAT (3 * IPC_DELTACAP)  // 3 mm detection/hold band

// FLEX-FLEX pairs (types 0/1) measure their gap at the MIDSURFACE: mjc_conGap does not subtract
// the radii for those types. The rest target is UNCHANGED -- still the standoff delta = min(ghc,
// IPC_DELTACAP) above -- only the origin the gap is measured from moves. Reason: subtracting
// r1 + r2 drives the gap permanently negative wherever the mesh geometry is tighter than the
// combined radii, as when a drawstring is threaded through a bag's hem, and addCand then DISCARDS
// those pairs as invalid -- no CCD coverage and no contact force, so the region can tunnel.
// Measured at the midsurface the same pairs read a healthy positive distance, clear the standoff,
// and stay admitted and force-carrying. Broad-phase reach is preserved by adding r1 + r2 back into
// the detection band only (see bandOffset in addCand).
