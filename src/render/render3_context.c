...
#include "render/render3_context.h"
...
const char *fragmentWireframeShaderSource = "#version 330 core\n"
    "out vec4 FragColor;\n"
    "void main()\n"
    "{\n"
...
static void makePlane(const mjModel* m, mjrContext* con) {
  ...
  /* GLuint shaderProgram = mjr_createShaderProgram(vertexShaderSource, fragmentShaderSource); */
  ...
  int vertsPerSide = numQuads + 1;
  int totalVerts = vertsPerSide * vertsPerSide;
  int totalQuads = numQuads * numQuads;
  int totalIndices = totalQuads * 6; // 2 triangles per quad, 3 vertices per triangle
  con->planeIndexCount = (unsigned int)totalIndices;

  // Allocate vertex data: position (3 floats) + normal (3 floats)
  float* vertices = (float*)mju_malloc(totalVerts * 3 * sizeof(float));

  double d = 2.0 / numQuads;
  int vertIndex = 0;
  ...
  for (int y = 0; y <= numQuads; y++) {
    for (int x = 0; x <= numQuads; x++) {
      // Position
      vertices[vertIndex++] = 0.5*(d * x - 1.0f); // x
      vertices[vertIndex++] = 0.5*(d * y - 1.0f); // y
      vertices[vertIndex++] = 0.0f;         // z

      /* // Normal (pointing up in +Z direction) */
      /* vertices[vertIndex++] = 0.0f;         // nx */
      /* vertices[vertIndex++] = 0.0f;         // ny */
      /* vertices[vertIndex++] = 1.0f;         // nz */
    }
  }

  // Allocate indices for triangles
  unsigned int* indices = (unsigned int*)mju_malloc(totalIndices * sizeof(unsigned int));

  int indexPos = 0;
  for (int y = 0; y < numQuads; y++) {
    for (int x = 0; x < numQuads; x++) {
      // Calculate vertex indices for this quad
      unsigned int bottomLeft  = y * vertsPerSide + x;
      unsigned int bottomRight = y * vertsPerSide + (x + 1);
      unsigned int topLeft     = (y + 1) * vertsPerSide + x;
      unsigned int topRight    = (y + 1) * vertsPerSide + (x + 1);
  ...
  /* printf("Init VAO index %i \n", (unsigned int)VAO); */
  glGenVertexArrays(1, &con->planeVAO);
  glGenBuffers(1, &con->planeVBO);
  glGenBuffers(1, &con->planeEBO);

  glBindVertexArray(con->planeVAO);

  glBindBuffer(GL_ARRAY_BUFFER, con->planeVBO);
  glBufferData(GL_ARRAY_BUFFER, 3*totalVerts*sizeof(float), vertices, GL_STATIC_DRAW);

  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, con->planeEBO);
...
void mjr3_makeContext_offSize(const mjModel* m, mjrContext* con, int fontscale,
  ...
  if (!con->glInitialized) {
    ...
    if (!mjgladLoadGL()) {
      mju_error("Failed to initialize GLAD\n");
    }

    // Print what GLAD thinks the version is
    printf("GLAD_GL_VERSION_3_3: %d\n", GLAD_GL_VERSION_3_3);
    /* printf("GLAD_GL_VERSION_4_1: %d\n", GLAD_GL_VERSION_4_1); */

    if (!GLAD_GL_VERSION_3_3) {
      mju_error("OpenGL version 3.0 or higher required");
    }

    GLint major, minor;
    glGetIntegerv(0x821B, &major);  // GL_MAJOR_VERSION = 0x821B
    glGetIntegerv(0x821C, &minor);  // GL_MINOR_VERSION = 0x821C
    // Not requires in version 3.0
    ...
    if (version && renderer) {
      ...
      mju_error("Failed to retrieve OpenGL version");
    }
  ...
  // Make shaders
  con->defaultShader = mjr_createShaderProgram(vertexShaderSource, fragmentShaderSource);
  con->wireframeShader = mjr_createShaderProgram(vertexShaderSource, fragmentWireframeShaderSource);
...
void mjr3_makeContext(const mjModel* m, mjrContext* con, int fontscale) {
  mjr3_makeContext_offSize(m, con, fontscale, 800, 600);
}
