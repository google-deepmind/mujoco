...
static void initGL3(const mjvScene* scn, const mjrContext* con) {
  ...
  glDepthMask(GL_TRUE);
  glDepthFunc(GL_GEQUAL);         // or GL_LEQUAL if not reversed depth
  glDepthRange(0.0, 1.0);
  glClearDepth(0.0);              // matches GL_GEQUAL
  ...
  glPolygonMode(GL_FRONT_AND_BACK,
                scn->flags[mjRND_WIREFRAME] ? GL_LINE : GL_FILL);
  glLineWidth(con->lineWidth);
...
/* // render */
void mjr3_render(mjrRect viewport, mjvScene* scn, const mjrContext* con) {
...
/*   mjvLight *thislight; */
  ...
  mat4 proj;
  float view[16];
  mjr3_lookAt(cam.pos, cam.forward, cam.up, view);
  ...
  if(cam.orthographic) {
    ...
    // Perspective projection from frustum
    glm_frustum(
                cam.frustum_center - halfwidth,
                cam.frustum_center + halfwidth,
                cam.frustum_bottom,
  ...
  // Send to shaders
  glUseProgram(con->defaultShader);
  GLuint viewLoc = glGetUniformLocation(con->defaultShader, "uView");
  GLuint projectionLoc = glGetUniformLocation(con->defaultShader, "uProjection");
  glUniformMatrix4fv(viewLoc, 1, GL_FALSE, (const GLfloat *)view);
  glUniformMatrix4fv(projectionLoc, 1, GL_FALSE, proj[0]);
  glBindVertexArray(con->planeVAO);
  glDisable(GL_DEPTH_TEST);
  glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
  glDrawElements(GL_TRIANGLES, con->planeIndexCount, GL_UNSIGNED_INT, 0);

  glUseProgram(con->wireframeShader);
  viewLoc = glGetUniformLocation(con->defaultShader, "uView");
  projectionLoc = glGetUniformLocation(con->defaultShader, "uProjection");
  glUniformMatrix4fv(viewLoc, 1, GL_FALSE, (const GLfloat *)view);
  glUniformMatrix4fv(projectionLoc, 1, GL_FALSE, proj[0]);
  glBindVertexArray(con->planeVAO);
  glDisable(GL_DEPTH_TEST);
