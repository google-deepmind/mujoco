...
void mjr3_lookAt(const float* eye, const float* forward, const float* up, float* mat) {
...
GLuint mjr_compileShader(GLenum type, const char* source) {
...
// Utility to compile + link a program
GLuint mjr_createShaderProgram(const char* vertex_src, const char* fragment_src) {
    GLuint vertex_shader = mjr_compileShader(GL_VERTEX_SHADER, vertex_src);
    GLuint fragment_shader = mjr_compileShader(GL_FRAGMENT_SHADER, fragment_src);
