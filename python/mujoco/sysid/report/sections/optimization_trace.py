...
class OptimizationTrace(ReportSection):
  ...
  def get_context(self) -> dict[str, Any]:
    ...
    return {
      "title": self._title,
      "objective_plot_html": (
        objective_fig.to_html(full_html=False, include_plotlyjs=False, config=config)
        if objective_fig
        else None
      ),
  ...
  def _get_objective_figure(self) -> go.Figure | None:
    ...
    fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor="rgba(211, 211, 211, 0.7)")
    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor="rgba(211, 211, 211, 0.7)")
    return fig
