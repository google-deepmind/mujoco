...
class AutomatedInsights(ReportSection):
  ...
  def __init__(
    ...
    self._n_warnings = len([i for i in self._insights if i["type"] == "warning"])
        ...
        insights.append(
          {
            ...
            "message": f"Parameter <b>{name}</b> ({val:.4g}) is at its lower bound ({lb:.4g}).",
          }
        )
        ...
        insights.append(
          {
            ...
            "message": f"Parameter <b>{name}</b> ({val:.4g}) is at its upper bound ({ub:.4g}).",
          }
        )
