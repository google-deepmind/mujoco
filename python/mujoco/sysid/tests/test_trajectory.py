...
from mujoco.sysid._src.trajectory import SystemTrajectory, create_initial_state
...
def sample_trajectory(mock_model):
  with mock.patch.object(SystemTrajectory, "check_compatible", return_value=None):
    ...
    state_mapping = {"qpos1": (timeseries.SignalType.MjStateQPos, np.array([0]))}
...
def test_save_and_load_with_signal_mapping(sample_trajectory, mock_model, tmp_path):
  ...
  for key in sensordata_mapping:
    ...
    np.testing.assert_array_equal(sensor_map[key][1], sensordata_mapping[key][1])
...
def test_create_initial_state(box_model):
  ...
  expected_size = mujoco.mj_stateSize(box_model, mujoco.mjtState.mjSTATE_FULLPHYSICS)
