...
def test_apply_pgain(arm_spec):
  ...
  modified_spec = model_modifier.apply_pgain(arm_spec, actuator_name, pgain_value)
...
def test_apply_dgain(arm_spec):
  ...
  modified_spec = model_modifier.apply_dgain(arm_spec, actuator_name, dgain_value)
  model = modified_spec.compile()
...
def test_apply_pdgain(arm_spec):
  ...
  modified_spec = model_modifier.apply_pdgain(arm_spec, actuator_name, pdgain_value)
  model = modified_spec.compile()
...
def test_apply_body_mass_explicit(arm_spec):
  ...
  np.testing.assert_allclose(model.body(body_name).inertia, original_inertia * scale)
...
def test_remove_visuals(arm_spec):
  ...
  assert len(cleaned_spec.textures) == 0
  assert len(cleaned_spec.materials) == 0
