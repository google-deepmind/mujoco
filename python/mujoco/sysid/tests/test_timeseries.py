...
from mujoco.sysid import SignalType, TimeSeries
...
def test_zero_column_data():
  ...
  data = np.empty((3, 0))
  ts = timeseries.TimeSeries(times=times, data=data)
  assert len(ts) == 3
...
def test_save_and_load(scalar_ts, multi_ts, tmp_path):
  ...
  loaded = timeseries.TimeSeries.load_from_disk(path)
  ...
  loaded2 = timeseries.TimeSeries.load_from_disk(path2)
...
def test_save_and_load_with_signal_mapping(tmp_path):
  ...
  signal_mapping = {
    "signal1": (timeseries.SignalType.MjSensor, np.array([0])),
    "signal2": (timeseries.SignalType.MjSensor, np.array([1])),
  }
  ts = timeseries.TimeSeries(times=times, data=data, signal_mapping=signal_mapping)
