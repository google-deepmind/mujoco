...
from mujoco.sysid._src.parameter import Parameter, ParameterDict
...
def _make_resample_ts(n_steps, n_cols, seed=0):
  ...
  mapping = {
    f"s{i}": (timeseries.SignalType.MjSensor, np.array([i])) for i in range(n_cols)
  }
  return timeseries.TimeSeries(times, data, signal_mapping=mapping)
...
def _run_both(ts, times, default_delay, sensor_delays, predicted_data):
  ...
  reference = signal_modifier._apply_resample_and_delay_columnwise(ts, times, delays)
...
def test_get_sensor_indices(arm_model):
  ...
  indices = signal_modifier.get_sensor_indices(arm_model, ["joint1_pos", "joint2_pos"])
...
def test_weighted_diff_with_weights(arm_model):
  ...
  result = signal_modifier.weighted_diff(predicted, measured, arm_model, weights)
...
class TestPatternMatching:
  ...
  def test_basic_glob(self):
    """Wildcard patterns select the right sensors (e.g. '*_pos' matches positions only)."""
  ...
  def test_last_match_wins(self):
    ...
    resolved = transform._resolve_delays(["joint1_torque", "joint5_torque"], pdict)
