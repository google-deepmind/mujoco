...
class CommonUtilsTest(absltest.TestCase):
  ...
  def test_uppercase_first_letter(self):
    ...
    self.assertEqual(uppercase_first_letter(" leading space"), " leading space")
  ...
  @mock.patch.dict(os.environ, {"BUILD_WORKSPACE_DIRECTORY": "/test_workspace"})
  def test_get_file_path(self):
    ...
    self.assertEqual(
        template_file,
        "/test_workspace/third_party/mujoco/wasm/codegen/templates/test.txt",
    )
    self.assertEqual(
        output_file,
        "/test_workspace/third_party/mujoco/wasm/codegen/generated/test.txt",
    )
